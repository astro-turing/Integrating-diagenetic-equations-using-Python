...
`pytables` is a new dependency and that caused problems across various platforms. This can be fixed for pipenv installs by installing pytables through conda: `conda activate py311; conda install -c anaconda pytables`.

Now you can use that freshly installed Python version and possibly any additionally installed libraries - using the `--site-packages` argument - by executing `pipenv install --python=$(conda run -n py311 which python) --site-packages --skip-lock`. The latter argument - `--skip-lock` may not be needed, but if your previous `pipenv install` failed, `pipenv --rm` may be needed. 
